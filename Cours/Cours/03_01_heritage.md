...
Choisir les objets que l'on veut utiliser pour une application est difficile.
(c'est même un métier en soi). Vous aurez des cours de POO pour vous apprendre
un peu mieux tout ca.

Prenons un exemple pour voir ce que la conception induit :
pour le moment, mes attaques spéciales (la boule de feu) est une méthode
spéciale du Magicien. et j'ai du surcharger la méthode *attaquer* pour que le
magicien utilise cette attaque en priorité.

Une autre conception aurait été possible (à vrai dire, beaucoup de conceptions
  auraient été possibles). Voici un autre choix, qui offre de nouvelles possibilités :

Un personnage a à sa disposition une liste d'attaques possibles.
Quand il attaque, il choisit une des attaques possibles et l'applique.

Dit comme ca, il semble qu'il faille une classe *Attaque*

Voici la nouvelle classe Attaque :
Un attaque est définie par un nom, un nombre de dégats,
et un texte qui sera prononcé quand on l'utilise.

Nous avons ajouté une méthode pour signaler si l'utilisateur peut ou pas utiliser
cette attaque.
```python
class Attaque():
    ...
    def isUsable(self, user):
        return True
```
Voyons comme le personnage utilise ces attaques :
un Personnage simple aura 2 attaques possibles :
- la baffe
- le kick
...
```
Voyez comme il est maintenant facile d'ajouter des attaques pour chaque personnage...
Regardons du coté du barbare :
Il va disposer des attaques normales, auxquelles viendront s'ajouter :
- la grosse taloche
- le coup de boule
...
Pour le magicien, ses attaques sont un peu spéciales, car il faut qu'il ait de la mana.
De plus, chaque attaque lui coute de la mana.
Nous devons donc créer une classe *AttaqueMagique* pour prendre en compte cela.
```python
from Attaque import Attaque
...
class AttaqueMagique(Attaque):
    ...
    def use(self, user, ennemi):
        if Attaque.use(self,user, ennemi) :
...
```
Et nous pouvons maintenant facilement signaler au magicien la liste de ses
attaques spéciales :
- La boule de feu
- l'éclair
...
class Magicien(Personnage):
    def __init__(self,nom):
        ...
        # je modifie ce qui m'intéresse
        self.mana = 5
        self.attaques.append(AttaqueMagique("Boule de feu", 5,"Burn, burn",4))
